...
import compstak.kafkastreams4s.vulcan.schema.{
  ...
  SerDesHelper,
  VulcanSchemaCodec,
  VulcanSchemaKeyCodec,
  VulcanSchemaSerdes,
  VulcanSchemaValueCodec
}
...
class AvroImplicits[F[_]: Effect](
  ...
  def avroSerdes[K: VCodec, V: VCodec]: AvroSerDes[F, K, V] =
    SerDesHelper.createAvroSerDes[F, K, V](schemaRegistryClient, properties)
  //User makes this^^ implicit.
  def toCodecVulcanSchema[K, V](
    a: AvroSerDes[F, K, V]
  ): (Codec[VulcanSchemaKeyCodec[K]], Codec[VulcanSchemaValueCodec[V]]) =
  ...
  def asCodecKey: Codec[VulcanSchemaKeyCodec] =
    new Codec[VulcanSchemaKeyCodec] {
      ...
      def serde[A: VulcanSchemaKeyCodec]: Serde[A] =
        VulcanSchemaSerdes.createKeySerDe[F, A](
          implicitly[Effect[F]],
          SerDesHelper
            .createAvroSerDes[F, A, A](schemaRegistryClient, properties)(
              implicitly[Sync[F]],
              implicitly[schema.VulcanSchemaKeyCodec[A]].vcodec,
              implicitly[schema.VulcanSchemaCodec[A]].vcodec
            )
            .key
        )
    }
  ...
  def asCodecValue: Codec[VulcanSchemaValueCodec] =
    new Codec[VulcanSchemaValueCodec] {
      ...
      def serde[A: VulcanSchemaValueCodec]: Serde[A] =
        VulcanSchemaSerdes.createValueSerDe[F, A](
          implicitly[Effect[F]],
          SerDesHelper
            .createAvroSerDes[F, A, A](schemaRegistryClient, properties)(
              implicitly[Sync[F]],
              implicitly[schema.VulcanSchemaValueCodec[A]].vcodec,
              implicitly[schema.VulcanSchemaValueCodec[A]].vcodec
            )
            .value
        )
  ...
  def asKeyCodec[T: VCodec]: VulcanSchemaKeyCodec[T] =
    new VulcanSchemaKeyCodec[T] {
      ...
      def serde: Serde[T] =
        VulcanSchemaSerdes.createKeySerDe[F, T](
          implicitly[Effect[F]],
          SerDesHelper.createAvroSerDes[F, T, T](schemaRegistryClient, properties).key
        )
    }
  ...
  def asValueCodec[T: VCodec]: VulcanSchemaValueCodec[T] =
    new VulcanSchemaValueCodec[T] {
      def vcodec = implicitly[VCodec[T]]
      def serde: Serde[T] =
        VulcanSchemaSerdes.createValueSerDe[F, T](
          implicitly[Effect[F]],
          SerDesHelper.createAvroSerDes[F, T, T](schemaRegistryClient, properties).value
        )
    }
